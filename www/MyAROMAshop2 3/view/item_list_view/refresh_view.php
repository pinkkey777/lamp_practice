            ...
            <h2>リフレッシュ</h2>
              <p><?php if(!empty($err_msg)){?>
             <?php foreach($err_msg as $value){?>
             <?php print $value ;}}?></p>
             <?php if(isset($_POST['in_cart'])){ ?>
             <p><?php if(!empty($result_msg)){?>
             <?php foreach($result_msg as $value){?>
             <?php print $value ;}}}?></p>
              <div class="flex">
               <?php 
               foreach($sales as $value){ 
                if($value['status'] === 0 && $value['purpose'] === 'refresh' ){?>
               <div class="menu">
            ...
            　 </div>
              ...
              <form method="post" action="top_add_cart.php">
                ...
                <input type="hidden" name="process_kind" value="refresh"/>
                </form>
                <?php if(isset($_POST['in_cart'])){ ?>
                <?php print '商品を追加しました'; }?>
    　</div>
